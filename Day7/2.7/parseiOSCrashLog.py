...
if __name__ == '__main__':
	...
	parse = ParseCrshLog()
	print "获取禅道bug id: %s的数据信息" %(bugId)
	domain = loginDomain
	parse.setLoginInfo(username,password,domain,bugId)
	parse.login()
	arr = parse.getLogInfos()
	if len(arr[0]) == 0:
		print "未获取到该bug id对应的影响版本的版本号"
		exit(1)
	print "版本号为: %s" %(arr[0])
	if len(arr[1]) == 0:
		print "该bug id对应的网址不包含crash日志文件"
		print "对应禅道地址为：%s" %(bugidurl + bugId)
		exit(1)
	print "crash日志文件地址为: %s" %(arr[1])
	...
	print "解析日志成功，名称为 readable.log, 中间文件及结果保存在\"桌面/%s/%s\"文件夹中" %(downloadsDir, bugId)
